			      ...
			      struct smb2_neg_context *ctxt, int ctxt_len)
{
	...
	pneg_ctxt = (void *)ctxt;
	/* The node supports RDMA transforms only on an SMB Direct transport. */
	if (!conn->transport->ops->rdma_read ||
	    !conn->transport->ops->rdma_write)
		return STATUS_SUCCESS;
			       ...
			       void *data, unsigned int datalen)
{
	...
	if (transform_type == SMB2_RDMA_TRANSFORM_TYPE_ENCRYPTION) {
		...
		err = ksmbd_sign_rdma(conn,
				      smb3_rdma_signing_key(work), data,
			       ...
			       size_t length, bool sync)
{
	...
	if (transform &&
		...
		ret = ksmbd_sign_rdma(work->conn,
				      ...
				      length, signature);
		if (ret || crypto_memneq(signature,
					 transform->crypto->Signature,
					 transform->signature_len)) {
			kvfree(data_buf);
			return ret ?: -EKEYREJECTED;
		}
	}
...
void smb2_set_sign_rsp(struct ksmbd_work *work)
{
	...
	if (hdr->Command == SMB2_READ && iov->iov_base != hdr) {
